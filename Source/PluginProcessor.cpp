...
{
    addParameter(midGain = new juce::AudioParameterFloat("midGain", "MidGain", 0.0f, 20.0f, 1.0f));
    addParameter(sideGain = new juce::AudioParameterFloat("sideGain", "SideGain", 0.0f, 20.0f, 1.0f));
}
...
void MidSideAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for(int i = 0; i < buffer.getNumSamples(); i++)
    {
        ...
        sampM = sampM * midGain->get();
        sampS = sampS * sideGain->get();
...
{
    std::unique_ptr<juce::XmlElement>xml(new juce::XmlElement ("MidGainBalance"));
    xml->setAttribute("MidGain", (double) midGain->get());
    xml->setAttribute("SideGain", (double) sideGain->get());
...
void MidSideAudioProcessor::setStateInformation (const void* data, int sizeInBytes)
{
    ...
    {
        if (xmlState->hasTagName ("MidGainBalance"))
        {
            *midGain = (float) xmlState->getDoubleAttribute ("MidGain", 1.0);
            *sideGain = (float) xmlState->getDoubleAttribute ("SideGain", 1.0);
        }
    }
}
